...
void MakeObjects()
{
	...
	for(int i=0; i<10; i++){
		if(i<4)
			body = CreateBody(220+40*i, MAX_Y-40, w, h, angle, isDynamic, isBall, density, restitution, friction);
		else if(i<7 && i>=4)
			body = CreateBody(240+40*(i-4), MAX_Y-80, w, h, angle, isDynamic, isBall, density, restitution, friction);
		else if(i<9 && i>=7)
			body = CreateBody(260+40*(i-7), MAX_Y-120, w, h, angle, isDynamic, isBall, density, restitution, friction);
		else
			body = CreateBody(280, MAX_Y-160, w, h, angle, isDynamic, isBall, density, restitution, friction);
	}
}
